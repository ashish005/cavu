...
@Component({
    template: `<a class="btn btn-xs b-theme text-xs border px-1 mx-1" (click)="showProjectProcessStages(context)">Workflow</a>`,
  standalone: false
})
export class ProjectPipelineSetupCell extends DynamicComponent {
    constructor(public apiResolver: ProjectAPIResolver) {
        super();
    }

    showProjectProcessStages(module: ProjectModule) {
        const {id, projectId, name} = module;
        const data = {
            //id: id,
            moduleId: id,
            projectId: projectId,
        ...
        this.apiResolver.showProjectWorkflowPopup(data, {text: `${name}`, desc: `Services for ${name}`});
    }
}

@Component({
    template: `<a class="text-primary text-xs" (click)="showProjectProcessStages(context)"> {{col?.hasWorkFlow ? 'Workflow': 'Change Status'}}</a>`,
  standalone: false
})
export class ProjectModuleWorkFlowCell extends DynamicComponent implements OnInit {
    constructor(public apiResolver: ProjectAPIResolver) {
        super();
    }
    ...
    showProjectProcessStages(module: ProjectModule) {
        ...
        if(this.col?.hasWorkFlow){
            this.apiResolver.showProjectWorkflowPopup(data, {text: `${name}`, desc: `Services for ${name}`});
        } else {
            this.apiResolver.showOrgStagesPopup(data, {text: `${name}`, desc: `Services for ${name}`});

            /*const popupOptions = {
                header: {text: `${name}`, desc: `Services for ${name}`},
                aside: ASIDE_CLASS.RIGHT,
                size: ASIDE_SIZE.W_50
            };
            //inputData.lookupMasterType = 'expense';
            const onSuccess = (resp)=> { this.sharedService.destroy(); };
            const onFailure = (resp)=> { this.sharedService.destroy(); };
            const modal$ = this.sharedService.showCustomPopup(StageCeComponent, popupOptions, data);
...
export class ProjectNameActionCell extends DynamicComponent {
    ...
    showDetails(row) {
      const { isClient } = this.activatedRoute.snapshot.data;
        if (isClient) {
            this.router.navigate(['project', row.id], {relativeTo: this.activatedRoute.parent.parent.parent});
            return
        }
...
@Component({
  standalone: false,
    template: `<div>
        <span class="text-xs">{{context.projectType}}</span>
        <div class="item-except text-xs h-1x"><span class="text-muted">{{context.division}}</span></div>
        <div><span class="text-xs">{{context.billingType }}</span></div>
    </div>`
})
export class ProjectTypeDivisionCell extends DynamicComponent {
    constructor(private router: Router, public activatedRoute: ActivatedRoute) {
        super();
    }
