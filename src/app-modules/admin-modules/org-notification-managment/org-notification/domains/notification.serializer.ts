...
export class OrgReminder {
    ...
    constructor(model: any = <any>{}) {
        const {
            id,
            orgTaskId, userGroupId, notificationId,
            frequencyType, reminderValue, name, userGroupName,
            status
        } = model;
        this.id = id;
...
export class OrgNotification {
    ...
    constructor(model: any = <any>{}){
        ...
        this.reminders = (reminders || []).map(r => new OrgReminder(r));
        this.orgTaskScheduleId = orgTaskScheduleId;
        this.nextRun = nextRun ? new NextScheduledRun(nextRun): null;

        this.hasTemplate = this.templates.some(r => r.hasTemplate);
        this.hasReminder = (this.orgTaskScheduleId && this.reminders?.length) ? true: false;
    }
}
