...
import {NotificationReminder, NotificationReminderTemplate} from "../domains/notification-reminder.serializer";
        ...
        <div class="item-except text-xs h-1x">
            ...
            <ng-template ngFor let-template [ngForOf]="context.templates | sortBy:'asc': 'sortOrder'" let-i="index">
                <a class="d-inline-block px-1 text-center">
                    <span class="d-block">
                        <a class="btn btn-xs text-xs text-success" (click)="showTemplateToCreateEdit(template)">{{ template?.mediaTypeName }}
                        </a>
                    </span>
                </a>
            </ng-template>
...
export class ReminderNameActionCell extends DynamicComponent {
    constructor(public activatedRoute: ActivatedRoute, public lookupResolver: OrgNotificationAPIResolver) {
        super();
    }

    eventTaskCalendar(task: NotificationReminder) {
    ...
    checkActivity(task: NotificationReminder) {
    ...
    showTemplateToCreateEdit(row: NotificationReminderTemplate) {
