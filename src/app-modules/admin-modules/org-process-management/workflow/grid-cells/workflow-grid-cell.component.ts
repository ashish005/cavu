    ...
    template: `<div class="phase-column">
        <ng-container *ngFor="let phase of context.phases; let i = index">
            <div class="phase-micro-item" >
                <span class="dot"></span>
                <span class="label" [style.color]="phase.color"> {{ phase.name }}  <span class="b-a mx-1"> {{ phase.steps?.length }} Steps </span></span>
            </div>
        </ng-container>
    </div>
...
export class WorkflowGridCellComponent extends DynamicComponent {
    constructor() { super(); }
