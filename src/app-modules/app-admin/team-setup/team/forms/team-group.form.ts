...
export class TeamGroupForm {
    ...
    filterGroupForm(data: TeamUserGroupRule){
        return this.fb.group(<any>{
            ...
            userFilterTypeId: [data.userFilterTypeId, Validators.required],
            operator: [data.operator, Validators.required],
            value: [data.value, Validators.required],
            valueId: [data.valueId, Validators.required]
        });
    }
    ...
    get formFilterOptions() { return <FormArray>this.customForm.get('rules'); }

    populateUserGroup(data: TeamUserGroup){
        const { id, name, categoryId, userTypeId, hasDynamicRules, rules} = data;
    ...
    deleteFilterOptions(index: number){
        this.formFilterOptions.removeAt(index);
    }
