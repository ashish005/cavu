...
export class PaymentGateway {
    ...
    modeGatewayMapper: Array<PaymentModeGatewayMapper>;
    ...
    constructor(model: any = <any>{}){
        const {
            id, name, systemTypeId, isMobileWallet, isPOS, hasOptionalRefNo, identificationNo,
            ...
            modeGatewayMapper, serviceCharges, systemTypeMasterType
        } = model;
        this.id = id;
        this.name = name;
        ...
        this.modeGatewayMapper = (modeGatewayMapper || []).map(r => new PaymentModeGatewayMapper(r));
    ...
    public get activeGatewayMapper() { return (this.modeGatewayMapper || []).filter(r => r.status); }
}

export class PaymentGatewaySerializer {
  fromJson(json: any): PaymentGateway { return new PaymentGateway(json); }
