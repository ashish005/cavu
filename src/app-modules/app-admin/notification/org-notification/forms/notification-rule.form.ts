...
export class NotificationRuleForm {
    ...
    addToTemplateTypeFormRule(item: NotificationMediaTypeTemplate){ this.formTemplates.push(this.mediaTypeFormGroup(item)); }

    populateForm(info: OrgNotification) {
        const { name, appEvent, notificationTypeId, isEventBased, permissions, templates } = info;
