...
import {Phase, PhaseTransition} from "../../domains/org-workflow-node.serializer";
...
export class WorkflowCanvasComponent {
    @Input() phases: Phase[] = [];
    @Input() transitions: PhaseTransition[] = [];
    ...
    ngOnChanges() {
        ...
        const source = this.transitions && this.transitions.length
            ? this.transitions
            : this.generateTransitions(this.phases);

        this.links = source.map<Edge>(t => ({
            id: `l-${t.id}`,
