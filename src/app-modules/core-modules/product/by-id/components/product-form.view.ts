...
export class ProductFormComponent extends ProductForm implements OnInit
{
    ...
    @Input() id: any;
    @Input() set data(val:  ProductById) {
        super.populateProduct(val);
        this.onCategoryTypeChange(val?.categoryTypeId);
        this.updateCategory(val?.categoryMapper);
    }

    constructor(public override fb: FormBuilder, public service: ProductByIdService, public lookupResolver: ProductLookupResolver)
    {
        super(fb);
    }
    ...
    refreshProduct(id: string){
        if(id) {
            ...
            {
                this.service.product = resp.data;
