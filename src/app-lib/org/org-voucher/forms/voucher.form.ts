...
class VoucherFormExtender {
    ...
    constructor(public injector: Injector) {
        ...
        const setupService = injector.get(AppSetupService);

        const { orgConfig, options } = setupService.appSetup;
        this.orgOption = options;
        this.orgConfig = orgConfig;

        this.orgLookup = orgLookupService.getOrgLookup();
        this.systemCurrency = this.orgLookup.getOrgCurrency(orgConfig.currencyCode);
        //this.orgActiveBranch = this.orgLookup.getActiveBranch();
...
export class VoucherForm extends VoucherFormExtender {
    ...
    constructor(public fb: FormBuilder, public override injector: Injector) {
        ...
        const isItemInvoiceChange = ([prev, next]: [boolean, boolean]) => {
            if (prev != next) {
                ...
                this.refCreditSearchableAccounts = this.orgLookup.getSearchAccounts(isPrimaryCredit, masterType, next);
                this.refDebitSearchableAccounts = this.orgLookup.getSearchAccounts(!isPrimaryCredit, masterType, next);
