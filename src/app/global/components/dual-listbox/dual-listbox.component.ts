...
import {BasicListDomain, DualListfilterButtonsDomain} from "./basic-list.domain";
import {CommonModule} from "@angular/common";
import {DualListboxFilterComponent} from "./dual-listbox-filter.component";
...
@Component({
  selector: 'dual-list',
  templateUrl: './views/dual-listbox.html',
  styleUrls: ['./views/dual-listbox.scss'],
  standalone: true, imports: [CommonModule, FormsModule, DualListboxFilterComponent]
})
...
export class DualListComponent implements DoCheck, OnChanges {
  ...
  available:BasicListDomain;
  confirmed:BasicListDomain;

  sourceDiffer:any;
  destinationDiffer:any;

  leftPanelFilterOptions = {
    buttons:[
      { type: 'checkbox' ,widthClass: 'w-25', align:'float-left', text:'All', action: this.selectUnselectAll.bind(this), disabledCheck: this.hasDisabledCheckAll.bind(this)},
      { type: 'button', widthClass: 'w-25', text:'None', action: this.selectNone.bind(this), disabledCheck:this.hasDisabledAnySelected.bind(this)},
      { type: 'button', widthClass: 'w-50', text:'Add >', action: this.moveItem.bind(this), disabledCheck:this.hasEnabledPicks.bind(this)}
    ].map(r=> { return new DualListfilterButtonsDomain(r) })
  };

  rightPanelFilterOptions = {
    buttons:[
      { type: 'button', widthClass: 'w-50',  text:'< Remove', action: this.moveItem.bind(this), disabledCheck:this.hasEnabledPicks.bind(this)},
      { type: 'button', widthClass: 'w-25', text:'None', action: this.selectNone.bind(this), disabledCheck:this.hasDisabledAnySelected.bind(this)},
      { type: 'checkbox', widthClass: 'w-25', align:'float-right', text:'All', action: this.selectUnselectAll.bind(this), disabledCheck: this.hasDisabledCheckAll.bind(this)}
    ].map(r=> { return new DualListfilterButtonsDomain(r) })
  };

  hasEnabledPicks(data){
    return (data.pick.length === 0);
  }

  hasDisabledAnySelected(available_confirmed){
    return !this.isAnySelected(available_confirmed);
  }

  hasDisabledCheckAll(available_confirmed){
    return this.disabled || this.isAllSelected(available_confirmed);
  }

  private sorter = (a:any, b:any) => { return (a._name < b._name) ? -1 : ((a._name > b._name) ? 1 : 0); };

  constructor(private differs:IterableDiffers) {
    this.available = new BasicListDomain(DualListComponent.AVAILABLE_LIST_NAME);
    this.confirmed = new BasicListDomain(DualListComponent.CONFIRMED_LIST_NAME);
  }
  ...
  dragEnd(list:BasicListDomain = null) : boolean {
    if (list) {
  ...
  drag(event:DragEvent, item:any, list:BasicListDomain) {
  ...
  allowDrop(event:DragEvent, list:BasicListDomain) : boolean {
  ...
  drop(event:DragEvent, list:BasicListDomain) {
    if (event.dataTransfer.types.length && (event.dataTransfer.types[0] === this.id)) {
      event.preventDefault();
      this.dragLeave();
      this.dragEnd();
  ...
  private makeUnavailable(source:BasicListDomain, item:any) {
  ...
  moveItem(source:BasicListDomain, target:BasicListDomain, item:any = null, trueup = true) {
    let i = 0;
  ...
  shiftClick(event:MouseEvent, index:number, source:BasicListDomain, item:any) {
  ...
  selectUnselectAll(buttonOption: DualListfilterButtonsDomain,source:BasicListDomain){
  ...
  selectAll(source:BasicListDomain) {
    source.pick.length = 0;
    source.pick = source.sift.slice(0);
  }

  selectNone(source:BasicListDomain) {
    source.pick.length = 0;

    if(source.name === "available") {
      this.leftPanelFilterOptions.buttons[0].checked = false;
    } else if(source.name === "confirmed"){
      this.rightPanelFilterOptions.buttons[2].checked = false;
    }
  }

  isAllSelected(source:BasicListDomain) : boolean {
  ...
  isAnySelected(source:BasicListDomain) : boolean {
  ...
  private unpick(source:BasicListDomain) {
  ...
  clearFilter(source:BasicListDomain) {
    if (source) {
      source.picker = '';
      this.onFilter(source);
    }
  ...
  onFilter(source:BasicListDomain) {
