      ...
      .then(() => {
        ...
        return this.oauthService.silentRefresh()
          ...
          .catch(result => {
            ...
            let needsLogin = (errorResponsesRequiringUserInteraction.indexOf(result?.reason?.error) >= 0);

            switch (result?.type) {
              case 'silent_refresh_timeout':
              case 'silent_refresh_error':
                needsLogin = true;
                console.warn('Session expired or failed to refresh silently.');
                break;
              default: break;
            }

            if (needsLogin) {
              this.oauthService.initLoginFlow(); // fallback: redirect to login
              // 3. ASK FOR LOGIN:
